...
const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL + "/agentcore/invoke"; // Your backend URL
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const response = await fetch(BACKEND_URL, {
        ...
        body: JSON.stringify({
          prompt: input, // Pass the user input as the prompt field
        }),
      });
      ...
      if (data?.body?.result?.content) {
        const assistantMessage = data.body.result.content[0].text; // Extract the message content from the backend response
